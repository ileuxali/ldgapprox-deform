...
namespace Draft2
{
  ...
  private:
    ...
    const FESystem<dim> fe_metriccon;
    DoFHandler<dim>   dof_handler;
    DoFHandler<dim>   metric_dof_handler;
    ...
    // variables for calculating stiffness matrix
    const double tau;
    const double mu;
    const double lambda;
  ...
  template <int structdim, int dim, int spacedim, bool level_dof_access>
  void
  HandlerChangeDoFAccessor<structdim, dim, spacedim, level_dof_access>::change_dof_handler(DoFHandler<dim, spacedim> *dh)
  {
    this->set_dof_handler(dh);
  }
  ...
  void BiLaplacianLDGLift<dim>::setup_system()
  {
    ...
    metric_dof_handler.distribute_dofs(fe_metriccon);
  ...
  void BiLaplacianLDGLift<dim>::assemble_matrix()
  {
    ...
    FullMatrix<double> stiffness_matrix_control(n_dofs,
                                           n_dofs); // interactions cell / cell for control matrix
                      ...
                      for (unsigned int j = 0; j < n_dofs; ++j)
                        {
                          ...
                          stiffness_matrix_cn(i, j) += (mu / 6) * scalar_product(g_inv_sqrt * H_j_neigh * g_inv_sqrt,
                                              g_inv_sqrt * H_i * g_inv_sqrt) * dx +
                                              (mu * lambda / (12 * mu + 6 * lambda)) * 
                                              scalar_product(g_inv_sqrt, H_j_neigh * g_inv_sqrt) * 
                                              scalar_product(g_inv_sqrt, H_i * g_inv_sqrt) * dx;
                          stiffness_matrix_nc(i, j) += (mu / 6) * scalar_product(g_inv_sqrt * H_j * g_inv_sqrt,
                                              g_inv_sqrt * H_i_neigh * g_inv_sqrt) * dx +
                                              (mu * lambda / (12 * mu + 6 * lambda)) * 
                                              scalar_product(g_inv_sqrt, H_j * g_inv_sqrt) * 
                                              scalar_product(g_inv_sqrt, H_i_neigh * g_inv_sqrt) * dx; 
                          stiffness_matrix_nn(i, j) += (mu / 6) * scalar_product(g_inv_sqrt * H_j_neigh * g_inv_sqrt,
                                              g_inv_sqrt * H_i_neigh * g_inv_sqrt) * dx +
                                              (mu * lambda / (12 * mu + 6 * lambda)) * 
                                              scalar_product(g_inv_sqrt, H_j_neigh * g_inv_sqrt) * 
                                              scalar_product(g_inv_sqrt, H_i_neigh * g_inv_sqrt) * dx;    
              ...
              for (unsigned int i = 0; i < n_dofs; ++i)
                {
                  for (unsigned int j = 0; j < n_dofs; ++j)
                    {
                      energy_matrix(local_dof_indices[i],
                            ...
                            for (unsigned int j = 0; j < n_dofs; ++j)
                              {
                                ...
                                stiffness_matrix_n1n2(i, j) += (mu / 6) * scalar_product(g_inv_sqrt * H_j_neigh2 * g_inv_sqrt,
                                              g_inv_sqrt * H_i_neigh * g_inv_sqrt) * dx +
                                              (mu * lambda / (12 * mu + 6 * lambda)) * 
                                              scalar_product(g_inv_sqrt, H_j_neigh2 * g_inv_sqrt) * 
                                              scalar_product(g_inv_sqrt, H_i_neigh * g_inv_sqrt) * dx;
                                stiffness_matrix_n2n1(i, j) += (mu / 6) * scalar_product(g_inv_sqrt * H_j_neigh * g_inv_sqrt,
                                              g_inv_sqrt * H_i_neigh2 * g_inv_sqrt) * dx +
                                              (mu * lambda / (12 * mu + 6 * lambda)) * 
                                              scalar_product(g_inv_sqrt, H_j_neigh * g_inv_sqrt) * 
                                              scalar_product(g_inv_sqrt, H_i_neigh2 * g_inv_sqrt) * dx;   
                              }
                        }
